...
static func build(
	...
	feet_align_y: float = -0.46,
	helmet_color: Color = Color(0.96, 0.82, 0.14)
) -> void:
	...
	if with_helmet:
		...
		hmat.albedo_color = helmet_color
		hmat.roughness = 0.4
		hmat.metallic = 0.15
		...
		bmat.albedo_color = helmet_color.darkened(0.08)
		bmat.roughness = 0.5
		brim.set_surface_override_material(0, bmat)
		avatar_root.add_child(brim)

	avatar_root.position.y = feet_align_y
