...
## Polling (physics frame) — works when `_input` never fires (embedded host / GUI swallowing).
var _prev_esc_down := false
var _prev_space_down := false
var _prev_e_down := false
var _prev_lmb_down := false
var _prev_rmb_down := false
var _prev_mouse_screen := Vector2.ZERO
...
func _ready() -> void:
	...
	_reset_sword_pose()
	call_deferred("_bootstrap_window_input")
	_prev_mouse_screen = get_viewport().get_mouse_position()
...
func _bootstrap_window_input() -> void:
	if _inside_base:
		return
...
func _apply_mouse_look(rel: Vector2) -> void:
	if rel.length_squared() < 0.0000001:
		return
	rotate_y(-rel.x * MOUSE_SENS)
	_spring_arm.rotate_x(-rel.y * MOUSE_SENS)
	var pitch := _spring_arm.rotation_degrees.x
	_spring_arm.rotation_degrees.x = clampf(pitch, PITCH_MIN, PITCH_MAX)


func _poll_mouse_look_fps(_delta: float) -> void:
	if Input.mouse_mode == Input.MOUSE_MODE_CAPTURED:
		var vel := Input.get_last_mouse_velocity()
		_apply_mouse_look(vel * _delta)
	else:
		var mp := get_viewport().get_mouse_position()
		var rel := mp - _prev_mouse_screen
		_prev_mouse_screen = mp
		_apply_mouse_look(rel)


func _poll_commander_mouse_yaw() -> void:
	if Input.mouse_mode == Input.MOUSE_MODE_CAPTURED:
		return
	var mp := get_viewport().get_mouse_position()
	var rel := mp - _prev_mouse_screen
	_prev_mouse_screen = mp
	_commander_yaw -= rel.x * MOUSE_SENS


func _poll_fps_mouse_buttons() -> void:
	var lmb := Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT)
	var lmb_edge := _edge_from_state(lmb, _prev_lmb_down)
	_prev_lmb_down = lmb
	if not lmb_edge:
		return
	if Input.mouse_mode != Input.MOUSE_MODE_CAPTURED:
...
func _poll_commander_mouse_buttons() -> void:
	var lmb := Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT)
	var rmb := Input.is_mouse_button_pressed(MOUSE_BUTTON_RIGHT)
	var lmb_edge := _edge_from_state(lmb, _prev_lmb_down)
	var rmb_edge := _edge_from_state(rmb, _prev_rmb_down)
	_prev_lmb_down = lmb
	_prev_rmb_down = rmb
	if not GameState.awaiting_tower_placement:
		return
	if rmb_edge:
		GameState.cancel_tower_blueprint()
		return
	if lmb_edge and get_viewport().gui_get_hovered_control() == null:
		var gp := _commander_ground_hit(get_viewport().get_mouse_position())
		if gp != null:
			GameState.try_place_tower(gp)


func _commander_ground_hit(screen_pos: Vector2):
...
func _physics_process(delta: float) -> void:
	...
	if _inside_base:
		...
		if _edge_from_state(e_down, _prev_e_down):
			_exit_commander_mode()
		_prev_e_down = e_down
		_poll_commander_mouse_buttons()
		return

	_poll_mouse_look_fps(delta)
	_poll_fps_mouse_buttons()

	if not is_on_floor():
		velocity.y -= GRAVITY * delta

	var space_down := Input.is_physical_key_pressed(KEY_SPACE)
	if is_on_floor() and _edge_from_state(space_down, _prev_space_down):
...
func _read_move_input() -> Vector2:
	# Do not rely on default InputMap entries (minimal project.godot may omit ui_* actions).
	...
	x += float(Input.is_physical_key_pressed(KEY_RIGHT)) - float(Input.is_physical_key_pressed(KEY_LEFT))
	y += float(Input.is_physical_key_pressed(KEY_DOWN)) - float(Input.is_physical_key_pressed(KEY_UP))
